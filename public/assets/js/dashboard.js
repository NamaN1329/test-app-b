(function ($) {
  ...
  $(function () {
    ...
    function getChartData() {
      const slots = JSON.parse(document.getElementById('slots')?.value || null);

      if (slots !== null) {
        const titles = slots.map(item => item.title);
        ...
        titles.forEach((title) => {
          setTimeout(() => {
            fetch('/post/' + title, {
              ...
              .then(responseData => {
                const id = responseData.data[0].title;
                const data = responseData.data;

                const groupedData = data.reduce((acc, curr) => {
                  const existingItem = acc.find(item => item.number === curr.number);
                  if (existingItem) {
    ...
    function createChart(data, id) {
      const marketingOverviewCanvas = document.getElementById('marketingOverview' + id);
      const lables = data.map(item => item.number);
